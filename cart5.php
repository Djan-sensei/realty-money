...
if(
	...
	$priz_c5 = $a_pok['stavka']*2000;
	if($bank_pol < $priz_c5) {
		$ccart3 = mysqli_query($mysqli, "UPDATE `poker` SET `cart5`='".$smen_cart."' WHERE `id`='".$a_pok['id']."'");
...
// СЕТ - Если 3 карты имеют одинаковый ранг и любую масть
	...
	$priz_c5 = round($a_pok['stavka']*2.5);
	if($bank_pol < $priz_c5) {
		$ccart3 = mysqli_query($mysqli, "UPDATE `poker` SET `cart5`='".$smen_cart."' WHERE `id`='".$a_pok['id']."'");
		echo '<script type="text/javascript"> window.location.href="poker.php"; </script>';
		exit;
...
// ДВЕ ПАРЫ - Если 2 карты одного ранга и 2 карты другого ранга из 5 имеют одинаковый ранг и любую масть
	...
	$priz_c5 = round($a_pok['stavka']*1.5);
	if($bank_pol < $priz_c5) {
		$ccart3 = mysqli_query($mysqli, "UPDATE `poker` SET `cart5`='".$smen_cart."' WHERE `id`='".$a_pok['id']."'");
		echo '<script type="text/javascript"> window.location.href="poker.php"; </script>';
		exit;
...
// ПАРА - Если 2 карты из 5 имеют одинаковый ранг и любую масть
	...
	$priz_c5 = round($a_pok['stavka']*0.5);
	if($bank_pol < $priz_c5) {
		$ccart3 = mysqli_query($mysqli, "UPDATE `poker` SET `cart5`='".$smen_cart."' WHERE `id`='".$a_pok['id']."'");
		echo '<script type="text/javascript"> window.location.href="poker.php"; </script>';
		exit;
