<?php
	...
	$konk = mysqli_fetch_array(mysqli_query($mysqli, "SELECT * FROM `konk` WHERE `end`='0' and `type`='1'"));
	
	if(date('Y-m-d H:i:s') >= $konk['finish'] and $konk['id']) {
		
		// Начисление призов
		$x=0;
		$sql_n = mysqli_query($mysqli, "SELECT *, SUM(`money`) as `sum` FROM `dvizh` WHERE `info`='Начислено с реферала 1 уровня' and `date`>='".$konk['start']."' GROUP by `id_users` ORDER by `sum` DESC LIMIT 0,3");
...
<head>
	...
	<title>RealtyMoney - Конкурс рефералов</title>
	<?php include('css.php'); ?>
</head>
<body>
    ...
    <div class="am-mainpanel">
		<div class="am-pagetitle">
			<h5 class="am-title">Конкурс рефералов</h5>
		</div>
				...
				<div class="row row-sm">
					<div class="col-lg-12">
						<div class="card pd-20">
							<h6 class="tx-12 tx-uppercase tx-inverse tx-bold mg-b-15">Конкурс рефералов №<?php echo $konk['id']; ?> с призовым фондом <?php echo $konk['priz1']+$konk['priz2']+$konk['priz3']; ?> руб.</h6>
							<p class="mg-b-20 mg-sm-b-30">В конкурсе учитываются только активные рефералы, зарегистрированные после запуска конкурса. Каждое пополнение вашего реферала, увеличивает шанс выигрыша!</p>
							...
							<p class="mg-b-20 mg-sm-b-30">
								<table class="table table-bordered">
									<tbody>
										...
										<?php
											$x=0;
											$sql_n = mysqli_query($mysqli, "SELECT *, SUM(`money`) as `sum` FROM `dvizh` WHERE (`info`='Начислено с реферала 1 уровня' or `info`='Начислено с реферала 2 уровня' or `info`='Начислено с реферала 1 уровня (серфинг)') and `date`>='".$konk['start']."' GROUP by `id_users` ORDER by `sum` DESC LIMIT 0,10");
											while($row_n = mysqli_fetch_assoc($sql_n)) {
												$x++;
										...
										?>	
												<tr>
													...
													<td><?php echo round($row_n['sum'], 2);?></td>
													<td><?php
